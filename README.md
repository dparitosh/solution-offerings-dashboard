...
1. Choose one or more detail sheets from the same quarter. Headers are detected within the first 25 rows. On **Map columns**, check suggested matches and sample values for each sheet. Exact names take priority; ambiguous aliases remain empty. Numeric amounts must be USD millions.
...
3. Group tags/peers by Offering or Sub-offering. Parent–child groups by Updated Offering. The same relationship and grouping apply across selected sheets. Advanced options let you change each sheet's header and inclusive row range, or copy matching column choices to the other selected sheets. Unmatched choices remain empty. Optional owner and remedial-plan mappings are collapsed to keep the main form short.
...
Apply replaces the selected quarter with the combined detail rows from every selected sheet in one transaction. It rejects duplicate rows across sheets, missing/invalid metrics, cyclic or conflicting parents, mixed quarter values when a Quarter column is present, and action links that would become orphaned or ambiguous. Existing action IDs and details are preserved when their source rows can be matched; parent-only actions must have a single destination parent. New detail rows distribute metrics equally across regions; matched details retain their regional proportions. Relationship labels and original source sheet/row provenance are saved and included in Excel exports. Use dashboard-export mode to import regional and action-registry sheets; custom mode combines detail sheets only, without summing duplicate records.
